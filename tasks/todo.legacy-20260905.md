...
> 현재 우선순위와 요구사항은 [개정 문서](todo.md)를 따릅니다. 원문의 체크 상태와 증거를 보존했습니다.
...
# 역사적 시점 유니버스·생존편향 제거 작업 목록

## Task HB01: 역사적 source와 백테스트 정책 계약

**설명:** 최초 지원 기간·종목 범위·상폐수익 정책을 승인하고 KRX/KIND 및 키움의 실제
응답을 민감정보 없는 fixture로 고정한다.

**Acceptance criteria:**
- [ ] 상폐 종목·시장 이전·재상장·거래정지의 처리 규칙이 문서화된다.
- [ ] 키움 일봉 TR, 연속조회, 조정주가, 호출 한도가 sample response로 검증된다.
- [ ] 상용 데이터 사용 여부와 라이선스 책임자가 정해진다.

**Verification:**
- [ ] fixture parser/source 단위 테스트가 통과한다.
- [ ] 5~10개 표본 종목의 KRX/KIND 원문과 수집 결과를 대조한다.

**Dependencies:** 없음
**Files likely touched:** `docs/`, `tests/fixtures/`, `tests/unit/`
**Estimated scope:** M

## Task HB02: Lifecycle event와 membership interval schema

**설명:** canonical instrument마다 상장·상폐·시장 이전·거래정지 이벤트와 근거를 저장하고,
유효기간 기반의 point-in-time membership을 조회할 수 있게 한다.

**Acceptance criteria:**
- [ ] code 재사용이 별도의 instrument·유효기간으로 저장된다.
- [ ] observed/inferred/unknown 상태와 source evidence가 보존된다.
- [ ] 기존 가격·RS FK는 변경하거나 삭제하지 않는다.

**Verification:**
- [ ] migration smoke test와 lifecycle repository unit test가 통과한다.
- [ ] listing 이후·delisting 이전 날짜만 membership으로 반환된다.

**Dependencies:** HB01
**Files likely touched:** `alembic/versions/`, `app/models/`, `app/repositories/`, `tests/unit/`
**Estimated scope:** M

## Task HB03: KRX/KIND 역사 이벤트 ingest와 reconciliation

**설명:** 상장·상폐 원문과 상태 변경을 lifecycle event로 적재하고, 현재 instrument 및
provider symbol과 안전하게 reconcile한다.

**Acceptance criteria:**
- [ ] raw source ID·checksum·수집 시각·근거 URL이 저장된다.
- [ ] 모호한 code/name 매칭은 자동 반영되지 않고 review 대상으로 남는다.
- [ ] 재실행은 idempotent하다.

**Verification:**
- [ ] 정상·정정·중복·code 재사용 fixture가 통과한다.
- [ ] 표본 상폐 공시의 상장일·상폐일이 저장값과 일치한다.

**Dependencies:** HB01, HB02
**Files likely touched:** `app/crawler/`, `app/services/`, `app/repositories/`, `tests/`, `scripts/`
**Estimated scope:** M

## Task HB04: Historical membership materialization과 coverage

**설명:** lifecycle event를 기준으로 거래일별 유니버스를 생성하고 상태별 coverage·누락을
보고한다.

**Acceptance criteria:**
- [ ] strict mode는 unknown membership을 포함하지 않는다.
- [ ] 현재 `symbols.is_active`가 과거 상태를 대체하지 않는다.
- [ ] 날짜·시장별 observed/inferred/unknown 비율을 조회할 수 있다.

**Verification:**
- [ ] 상장 전·상장 중·상폐 후·재상장 fixture가 통과한다.
- [ ] coverage report가 snapshot과 event 근거를 연결한다.

**Dependencies:** HB02, HB03
**Files likely touched:** `app/services/`, `app/repositories/`, `app/api/`, `tests/`
**Estimated scope:** M

## Checkpoint HB1: 역사적 유니버스 승인

- [ ] 상폐·재상장·시장 이전 표본이 원문과 대조됐다.
- [ ] 최초 지원 기간의 membership coverage 목표와 unknown 처리 정책이 승인됐다.
- [ ] 전체 테스트와 migration smoke test가 통과한다.

## Task HB05: 키움 canary와 범위 제한 OHLC backfill

**설명:** 검증된 키움 차트 계약을 사용해 지정 기간의 OHLC만 checkpoint 단위로 upsert하고,
source lineage·실패 목록·재개 위치를 남긴다.

**Acceptance criteria:**
- [ ] 시작·종료일과 대상 instrument가 명시된 job만 실행된다.
- [ ] source throttle·partial response·재시도 한도가 적용된다.
- [ ] Naver와 충돌한 가격은 자동 덮어쓰기 대신 validation case가 된다.

**Verification:**
- [ ] canary 5~10종목에서 응답 깊이·수정주가·continuation을 확인한다.
- [ ] 중단 후 checkpoint로 재개해 중복 없이 같은 결과를 얻는다.

**Dependencies:** HB01, Checkpoint HB1
**Files likely touched:** `app/crawler/`, `app/services/`, `scripts/`, `tests/`
**Estimated scope:** M

## Task HB06: Historical data-quality와 exclusion policy

**설명:** 결측·거래정지·OHLC 이상치·기업행위·공급자 충돌을 분류하고, strict backtest가
사용할 validated input을 만든다.

**Acceptance criteria:**
- [ ] exclusion은 reason, 유효기간, source evidence를 가진다.
- [ ] 행 삭제 없이 raw/canonical/validated 상태를 구분한다.
- [ ] 종목·날짜·시장별 데이터 coverage가 산출된다.

**Verification:**
- [ ] 각 결측·이상치·분할·충돌 fixture가 기대 상태로 분류된다.
- [ ] validated dataset은 exclusion 근거를 조회할 수 있다.

**Dependencies:** HB04, HB05
**Files likely touched:** `app/services/validation/`, `app/models/`, `app/api/`, `tests/`
**Estimated scope:** M

## Task HB07: Strict point-in-time backtest dataset API

**설명:** v2 backtest API가 해당 날짜의 membership, validated price, RS lineage 및 coverage를
반환하고, dataset version으로 재현성을 제공하게 한다.

**Acceptance criteria:**
- [ ] strict mode는 observed membership과 validated price만 반환한다.
- [ ] API는 기간 필수·cursor·page size·backtest:read scope를 유지한다.
- [ ] dataset metadata는 membership/price/RS source version을 식별한다.

**Verification:**
- [ ] 상폐된 종목이 상폐 전에는 포함되고 이후에는 제외되는 API 테스트가 통과한다.
- [ ] cursor 페이지를 합친 결과가 같은 dataset version 내에서 중복·누락이 없다.

**Dependencies:** HB04, HB06
**Files likely touched:** `app/api/`, `app/services/`, `app/schemas/`, `tests/`
**Estimated scope:** M

## Task HB08: Historical replay와 운영 승인

**설명:** 알려진 상폐·합병·시장 이전 종목으로 survivor-only 결과와 strict 결과를 비교하고,
coverage 한계 및 상폐수익 가정을 포함한 운영 결정을 기록한다.

**Acceptance criteria:**
- [ ] replay report가 기간·dataset ID·정책 version·coverage를 포함한다.
- [ ] survivor-only 대비 종목 수·수익 차이가 설명된다.
- [ ] 대량 backfill 실행 전 운영자가 범위·예상 시간·저장량을 승인한다.

**Verification:**
- [ ] replay integration test와 full pytest가 통과한다.
- [ ] canary 결과와 rollback 절차가 기록된다.

**Dependencies:** HB07
**Files likely touched:** `scripts/`, `app/services/`, `docs/`, `tests/`
**Estimated scope:** M
...
## Task R01: Legacy batch runner의 거래일 주입

**설명:** 테스트와 replay가 현재 날짜·휴장 여부에 좌우되지 않도록 `BatchContext`의 명시적
거래일을 보존한다.

**Acceptance criteria:**
- [x] 명시된 거래일의 정상 거래일 배치가 현재 날짜와 무관하게 실행된다.
- [x] 명시된 거래일이 없으면 기존 calendar 기반 기본 동작을 유지한다.

**Verification:**
- [x] `pytest -q tests/unit/test_market_calendar.py tests/integration/test_batch_harness.py`

**Dependencies:** 없음
**Files likely touched:** `app/services/batch/run_daily_job.py`, `tests/unit/test_market_calendar.py`, `tests/integration/test_batch_harness.py`
**Estimated scope:** S

## Task R02: Orchestrator 거래일 고정

**설명:** 시작 시 검증한 거래일을 KRX와 가격 단계까지 유지한다.

**Acceptance criteria:**
- [x] 실행 도중 calendar 함수 결과가 바뀌어도 단계 컨텍스트의 거래일은 시작 날짜다.
- [x] 휴장 판정과 단계 기준일이 동일하다.

**Verification:**
- [x] `pytest -q tests/unit/test_market_calendar.py`

**Dependencies:** R01
**Files likely touched:** `app/services/batch/orchestrator.py`, `tests/unit/test_market_calendar.py`
**Estimated scope:** S

## Task R03: Canary expand의 시간 순서 게이트

**설명:** 현재 결정일 이후의 continue가 expand 조건에 포함되지 않게 한다.

**Acceptance criteria:**
- [x] 현재 snapshot 날짜보다 이른 두 continue만 expand를 허용한다.
- [x] 미래 continue 두 건만 있는 경우 expand가 거부된다.

**Verification:**
- [x] `pytest -q tests/unit/test_universe_canary_decision.py`

**Dependencies:** 없음
**Files likely touched:** `app/services/universe_canary_decision.py`, `tests/unit/test_universe_canary_decision.py`
**Estimated scope:** S

## Checkpoint: Review fixes

- [x] `TELEGRAM_ENABLED=false NOTIFICATION_ENABLED=false .venv/bin/pytest -q`
- [x] `.venv/bin/python -m compileall -q app tests`
- [x] `git diff --check`
