...
> 2026-09-05 우선순위 개정: 최우선은 백테스트용 상장·상폐 이력, 과거 OHLC,
> 결측/이상치 검증과 재현 가능한 dataset 구축이다.
> 현재 실행 순서는 [백테스트 로드맵](roadmap_krx_universe.md)과 [TODO](../tasks/todo.md)를 따른다.
> 아래 Phase/완성도는 기존 개발 이력이며 역사 데이터 구축 완료를 의미하지 않는다.
> RS 산식 개선 및 최신 유니버스 운영 확대는 백테스트 데이터 구축 이후로 미룬다.
